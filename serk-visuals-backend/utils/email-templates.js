...
function customerConfirmation(booking) {
  const { name, type, date, durationMinutes, location, numberOfPeople, preferredContactMethod, estimatedPrice, message } = booking || {};
  const when = fmt(date);
...
• Group size:      ${numberOfPeople || "N/A"}
• Contact method:  ${preferredContactMethod || "N/A"}
• Est. price:      ${estimatedPrice ? `$${estimatedPrice}` : "N/A"}
• Notes:           ${message || "—"}
...
– Serk Visuals`;
  ...
  const row = (label, value) =>
    `<tr><td style="padding:8px;border-bottom:1px solid #eee;">${label}</td><td style="padding:8px;border-bottom:1px solid #eee;"><strong>${value}</strong></td></tr>`;
  ...
  <div style="font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;max-width:600px;margin:auto;padding:16px;color:#111;">
    ...
    <table style="border-collapse:collapse;width:100%;margin:12px 0;">
      ${row("Session type", type || "N/A")}
      ${row("Date / Time", when)}
      ${row("Duration", duration)}
      ${row("Location", location || "N/A")}
      ${row("Group size", numberOfPeople || "N/A")}
      ${row("Preferred contact", preferredContactMethod || "N/A")}
      ${estimatedPrice ? row("Est. price", `$${estimatedPrice}`) : ""}
      <tr><td style="padding:8px;border-bottom:1px solid #eee;">Notes</td><td style="padding:8px;border-bottom:1px solid #eee;">${(message || "—").toString().replace(/</g, "&lt;")}</td></tr>
    </table>
...
function adminNotification(booking) {
  const { name, email, phone, type, date, durationMinutes, location, numberOfPeople, preferredContactMethod, estimatedPrice, message, status, createdAt, _id } =
    booking || {};
  const when = fmt(date);
